...
# Date parsing helper
def parse_relative_date(time_phrase: str, reference_date: date = None) -> tuple[date, date, date, date]:
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            if api_endpoint == "/satellite/deforestation":
                ...
                # Validate bbox data
                if not all(k in bbox_data for k in ["min_lon", "min_lat", "max_lon", "max_lat"]):
                    return {
...
API_MAP = {
    ...
    "urban_heat": {"endpoint": "/satellite/urban_heat", "name": "Urban Heat Island Mapping"},
    "irrigation": {"endpoint": "/irrigation/detect", "name": "Agricultural Health Monitoring"},
}
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            if needs_more_info:
                return ChatResponse(
                    ...
                    metadata={
                        ...
                        "partial_recommendation": response_json.get("partial_recommendation", "")
                    },
            ...
            bbox = None
            if extracted_params.get("needs_geocoding") and extracted_params.get("location_extracted"):
            ...
            if recommended_apis:
                top_api = recommended_apis[0]
                final_response = f"**Recommended Analysis: {top_api.get('api_name')}**\n"
                final_response += f"Confidence Score: {top_api.get('relevance_score', 0):.1%}\n"
            ...
            return ChatResponse(
                ...
                metadata={
                    ...
                    "implementation_steps": response_json.get("implementation_steps", []),
                    ...
                    "alternative_applications": response_json.get("alternative_applications", [])
                },
...
async def execute_analysis(query: UserQuery):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if query.ref_start and query.ref_end and query.recent_start and query.recent_end:
                response = await client.post(
